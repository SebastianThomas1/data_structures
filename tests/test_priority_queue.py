...
class TestPriorityQueue(unittest.TestCase):
    ...
    def _get_instance_from_iterable(self, values):
        return self._tested_class.from_iterable(values, self._extreme_key)

    def _get_complementary_instance_from_iterable(self, values):
        return self._tested_class.from_iterable(
            values, self._complementary_extreme_key)

    def setUp(self):
        self.empty_queue = self._get_empty_instance()
        self.queue_length_1 = self._get_instance_from_iterable([0])
        self.range_queue = self._get_instance_from_iterable(range(4))
        self.queue = self._get_instance_from_iterable([1, 42, -3, 2, 42])

    def test_eq(self):
        self.assertEqual(self.empty_queue,
                         self._get_empty_instance())
        self.assertEqual(self.queue_length_1,
                         self._get_instance_from_iterable(range(1)))
        self.assertEqual(self.range_queue,
                         self._get_instance_from_iterable([0, 1, 2, 3]))
        self.assertEqual(self.queue,
                         self._get_instance_from_iterable((1, 42, -3, 2, 42)))

        self.assertNotEqual(self.empty_queue,
                            self._get_complementary_instance_from_iterable([]))
        self.assertNotEqual(
            self.queue_length_1,
            self._get_complementary_instance_from_iterable([0]))
        self.assertNotEqual(
            self.range_queue,
            self._get_complementary_instance_from_iterable(range(4)))
        self.assertNotEqual(
            self.queue,
            self._get_complementary_instance_from_iterable([1, 42, -3, 2, 42]))
        ...
        self.assertNotEqual(self.empty_queue, [])
        self.assertNotEqual(self.queue_length_1, range(1))
        self.assertNotEqual(self.range_queue, [0, 1, 2, 3])
    ...
    def test_delitem(self):
        ...
        self.assertEqual(self.range_queue, self._get_instance_from_iterable(
            [2, 1, 0] if self._extreme_key == MAX else [1, 2, 3]))
        del self.queue[self._extreme_key]
        self.assertEqual(self.queue, self._get_instance_from_iterable(
            [42, 2, 1, -3] if self._extreme_key == MAX else [1, 2, 42, 42]))
    ...
    def test_iadd(self):
        ...
        self.assertEqual(self.empty_queue,
                         self._get_instance_from_iterable([0]))
        self.assertEqual(self.queue_length_1,
                         self._get_instance_from_iterable([3, 2, 1, 0, 0]))
        self.assertEqual(self.range_queue, self._get_instance_from_iterable(
            [42, 42, 3, 2, 2, 1, 1, 0, -3]))
        self.assertEqual(self.queue, self._get_instance_from_iterable(
            [42, 42, 2, 1, 0, -3]))
        ...
        self.assertEqual(self.empty_queue,
                         self._get_instance_from_iterable([-1, -1, 0]))
        self.assertEqual(self.range_queue, self._get_instance_from_iterable(
            [-3, -3, -2, -1, 0, 1, 1, 2, 2, 3, 42, 42]))
        self.assertEqual(self.queue, self._get_instance_from_iterable(
            [-3, 0, 1, 2, 42, 42]))
    ...
    def test_post(self):
        ...
        self.assertEqual(self.empty_queue,
                         self._get_instance_from_iterable([-1]))
        self.assertEqual(self.queue_length_1,
                         self._get_instance_from_iterable([-2, -1, 0]))
        self.assertEqual(self.range_queue, self._get_instance_from_iterable(
            [3, 2, 1, 0, -1, -2, -3]))
        self.assertEqual(self.queue, self._get_instance_from_iterable(
            [-3, -3, -2, -1, 1, 2, 42, 42]))
    ...
    def test_pop(self):
        ...
        self.assertEqual(self.queue_length_1, self._get_empty_instance())
        self.assertEqual(self.range_queue, self._get_instance_from_iterable(
            [1, 0] if self._extreme_key == MAX else [2, 3]))
        self.assertEqual(self.queue, self._get_instance_from_iterable(
            [2, 1, -3] if self._extreme_key == MAX else [2, 42, 42]))
...
class TestArrayMinPriorityQueue(TestPriorityQueue):
    ...
    def _get_instance_from_iterable(self, values):
        return self._tested_class.from_iterable(values)

    def _get_complementary_instance_from_iterable(self, values):
        return ArrayMaxPriorityQueue.from_iterable(values)
...
class TestArrayMaxPriorityQueue(TestPriorityQueue):
    ...
    def _get_complementary_instance_from_iterable(self, values):
        return ArrayMinPriorityQueue.from_iterable(values)
...
class TestOrderedArrayMinPriorityQueue(TestPriorityQueue):
    def __init__(self, method_name):
        super().__init__(method_name=method_name,
                         tested_class=OrderedArrayMinPriorityQueue,
                         extreme_key=MIN)
    ...
    def _get_complementary_instance_from_iterable(self, values):
        return OrderedArrayMaxPriorityQueue.from_iterable(values)


class TestOrderedArrayMaxPriorityQueue(TestPriorityQueue):
    def __init__(self, method_name):
        super().__init__(method_name=method_name,
                         tested_class=OrderedArrayMaxPriorityQueue,
                         extreme_key=MAX)
    ...
    def _get_complementary_instance_from_iterable(self, values):
        return OrderedArrayMinPriorityQueue.from_iterable(values)
...
class TestHeapPriorityQueueMAX(TestArrayPriorityQueue):
    ...
    def test_init(self):
        self.assertEqual(self.empty_queue._values, [None])
        self.assertEqual(self.queue_length_1._values, [None, 0])
        self.assertEqual(self.range_queue._values, [None, 3, 1, 2, 0])
        self.assertEqual(self.queue._values, [None, 42, 42, -3, 2, 1])
...
class TestHeapMinPriorityQueue(TestPriorityQueue):
    ...
    def _get_complementary_instance_from_iterable(self, values):
        return HeapMaxPriorityQueue.from_iterable(values)
...
class TestHeapMaxPriorityQueue(TestPriorityQueue):
    ...
    def _get_complementary_instance_from_iterable(self, values):
        return HeapMinPriorityQueue.from_iterable(values)
